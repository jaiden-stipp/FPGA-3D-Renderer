module async_fifo #(
    ...
    (* ramstyle = "M9K" *) logic [WIDTH-1:0] memory [0:(1 << ADDRESS_WIDTH) - 1];
    ...
    function automatic logic [POINTER_WIDTH-1:0] gray_to_binary(
        ...
        begin
            ...
            for (bit_number = POINTER_WIDTH - 2; bit_number >= 0;
                 bit_number = bit_number - 1)
                gray_to_binary[bit_number] = gray_to_binary[bit_number + 1] ^
                                             value[bit_number];
        end
    endfunction
    ...
    assign write_full_next = write_gray_next == {
